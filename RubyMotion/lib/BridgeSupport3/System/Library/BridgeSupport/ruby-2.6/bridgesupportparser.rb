...
module Bridgesupportparser
	...
	def rename64!
	    ...
	    # When only a 64-bit parse runs, write both the plain and 64-suffixed
	    # attribute names so consumers of either work correctly. The VM
	    # (bs.c) prefers type64/value64 on 64-bit but falls back to
	    # type/value, and tests check the plain names.
	    @all_attrs.each do |attrs|
		Bridgesupportparser.rename_attrs64.each do |plain, suffixed|
		    attrs[plain] = attrs[suffixed] if attrs.include?(suffixed) && !attrs.include?(plain)
		end
	    end
	end
